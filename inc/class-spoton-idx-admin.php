			...
			case 'get_field':
				...
				if (!$fd->static)
				{
					...
					if (in_array($fd->title, array('State', 'County', 'City')))
					{
						...
						$query = 'RKEY eq ' . $spoton_pkey . '&$orderby=' . $entity_ppt;
					}
					else
					{
						// Available values, get from Odata
	...
	protected function build_forms()
	{
		...
		$form = array(
			...
			'input' => array(
				...
				'provider_key' => array('size' => 50, 'label' => __('', $this->domain)),
		...
		$form = array(
			'items'	=> array('heading', 'checkbox','checkbox', 'checkbox', 'checkbox'),
			'item_labels' => array(
			...
			'item_names'	=> array('h1', 'cb1', 'cb2', 'cb3', 'cb4'),
			'heading' => array(
