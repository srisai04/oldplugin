<?php
	...
	$ln = !empty($spoton_page) ? $spoton_page : 1;
	 ...
	 $Provider_Key = (strlen($spoton_pkey) >6) ? $spoton_pkey : "guid'AE881A9B-A890-406C-B99D-DAAAA9484A03'";
    
	$PKey = ' RKEY eq ' . $Provider_Key . ' and ';
	$query = $PKey . " LN eq '" . $ln . "'";
	...
	if($notFound==0)
    {
	    ...
	    $listingResponse = $proxy->GenericRetsDetailListings()->filter($query)->Select('PKey,LN,HouNo,Stre,DirPre,StrSuff,DirSuff,City,State,Zip,PropType,Status,SquFeet,Price,Acre,Beds,Baths,Loc,Remark,YrBuilt,Logo,AddrYN')->Top('1')->Execute();
...
function listing(ln,PKey)
		{  
		    ...
		    var adrress=""; var AgName=""; var Disclaimer="";var OffNam="";
            var addryn='';
            for (var i = 0; i < data.results.length; i++) 
            {
                ...
                jQuery('#spto_disclaimer').html('"Disclaimer :'+Disclaimer+'--"Last Updated Date:'+lastUpdate_date+'"');
