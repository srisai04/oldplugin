    ...
    {     
     ...
     function loadState()
		...
		function loadCounty(state)
		{
			...
			var query ="";
			if(state==0)
				query="$filter=RKEY eq "+ RKEY;
			else
				query="$filter=RKEY eq "+ RKEY +" and State eq '"+state+"'";   
		...
		function loadCity(state,county)
		{
			...
			if(state==0 && county==0)
				...
				query="$filter=RKEY eq "+ RKEY +" and State eq '"+jQuery("#spto_state").val()+"' and County eq '"+county+"'";
