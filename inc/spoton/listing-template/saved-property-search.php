	...
	$listingResponse = $proxy->GenericRetsSearchListings()->skip(strval($no))->Top(strval($top))->filter($query)->Select('PKey,LUD,Disclaimer,LN,HouNo,Stre,DirPre,StrSuff,DirSuff,City,State,Zip,PropType,Status,SquFeet,Price,Acre,Beds,Baths,ImgNam,Logo,OffNam,AgNam,AddrYN')->IncludeTotalCount()->Execute(); 
	...
	if (!empty($SearchListings->ImgNam)) { ?>
		<a id="imgLink" href=<?php echo spoton_get_ppt_permalink($SearchListings); ?>><img class="soidx-listing-thumbnail sepV_a" src=<?php echo str_replace('mapinfoboximages', 'mapthumbnailimages',str_replace('retsmapinfoboximages', 'retsmapthumbnailimages',$SearchListings->ImgNam)); ?>></a>
	<?php } else { ?>
		<a id="imgDefaultLink" href="<?php echo spoton_get_ppt_permalink($SearchListings); ?>"><img src="http://xpioimages.blob.core.windows.net/mapthumbnailimages/default.jpg"/></a>
	<?php } ?>
	</div>
...
<?php if($pkey == 3)
        ...
        <a id="imgDefaultLink" href="<?php echo spoton_get_ppt_permalink($SearchListings); ?>"><?php echo spoton_get_ppt_addr($SearchListings); ?></a>
        </h2>
              <?php } }
         else { ?>
...
<?php  $year=substr($lud,0,4);  $month=substr($lud,5,2); $day=substr($lud,8,2);
      $date=$month."/".$day."/".$year;
?>
    <div id="discl_info" style="color:black;"><?php echo '"Disclaimer :'.$Disclaimer.$date; ?></div> 
