...
function moreinfolisting(ln)
		{  
            ...
            host+='/property/mls-'; 
            var requestUri = serviceUri + "GenericRetsDetailListings?$filter=RKEY eq "+ PKey +" and (LN eq '" + ln +"')&$select=Logo,OffNam,Disclaimer,AgNam,IntFea,Price,WatFront,PropView,PropType,LN,Acre,YrBuilt,HouNo,HiSch,EleSch,JuHiSch,PropSub,SchDist,Stre,ExtFea,StrSuff,DirSuff,City,State,Zip,Remark,Style,AppThtStay,SquFeet";
...
function listing(ln)
		...
		{    
			...
			jQuery.ajax({
                dataType: 'jsonp',
                url: "http://realestateservice.cloudapp.net/OdataService.svc/RetsListingImages?$filter=( RKEY eq "+ PKey +" and LN eq '" + ln + "')&$inlinecount=allpages&$format=json",
                jsonp: '$callback',
        ...
        function addInfobox(e) 
        {
            ...
            if (e.targetType == "pushpin") 
            {
                ...
                if(splitloc[1]>middlewest)
                    x=-320;         
                var query ="$filter=RKEY eq "+ PKey +" and Lat eq "+ splitloc[0] +" and Lon eq " + splitloc[1];   
...
StrSuff+" "+data.results[i].DirSuff+" "+data.results[i].City+", "+data.results[i].State+" "+data.results[i].Zip; 
                        ...
                        var infoboxOptions =  {width :320, height :180, showCloseButton: true, zIndex: 1000,showPointer:false, offset:new Microsoft.Maps.Point(x,y),title:address, description:'<img  style="float:left;width:100px;height:75px;" src="'+image+'"/><span style="float:right;margin-top:0;">'+desc+'</span>',actions:[{label: 'Property Details', eventHandler:
