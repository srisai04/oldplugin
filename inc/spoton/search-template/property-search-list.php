...
function moreinfolisting(ln)
    {  
        ...
        host+='/property/mls-'; 
        var requestUri = serviceUri + "GenericRetsDetailListings?$filter=RKEY eq "+ PKey +" and (LN eq '" + ln +"')&$select=Logo,OffNam,Disclaimer,AgNam,IntFea,Price,WatFront,PropView,PropType,LN,Acre,YrBuilt,HouNo,HiSch,EleSch,JuHiSch,PropSub,SchDist,Stre,ExtFea,StrSuff,DirSuff,City,State,Zip,Remark,Style,AppThtStay,SquFeet,RKEY";
...
// jQuery("#inline_"+ln).html(property).show('slow');
    ...
    function loadCounty(state)
    {
        ...
        var query ="";
        if(state==0)
            query="$filter=RKEY eq "+ PKey;
        else
            query="$filter=RKEY eq "+ PKey +" and State eq '"+state+"'";   
    ...
    function loadCity(state,county)
    {
        ...
        if(state==0 && county==0)
            ...
            query="$filter=RKEY eq "+ PKey +"and State eq '"+jQuery("#spto_state").val()+"' and County eq '"+county+"'";
...
<?php }  ?>           
    ...
    {       
        ...
        multiLN=getMultipleLN();
        if(multiLN.length!=0 && submitClick==2)
              requestUri = serviceUri + "GenericRetsSearchListings?" + query +"$select=PKey,Disclaimer,LN,HouNo,DirPre,Stre,StrSuff,DirSuff,City,State,Zip,County,PropType,SquFeet,Price,Acre,Beds,Baths,CreDate,Disclaimer,ImgNam,"+AgName+"Logo,"+offname+"AddrYN,LUD,RKEY&$inlinecount=allpages";
        else    
              requestUri = serviceUri + "GenericRetsSearchListings?" + query +"$select=PKey,Disclaimer,LN,HouNo,DirPre,Stre,StrSuff,DirSuff,City,State,Zip,County,PropType,SquFeet,Price,Acre,Beds,Baths,CreDate,Disclaimer,ImgNam,"+AgName+"Logo,"+offname+"AddrYN,LUD,RKEY&" + "$skip="+String(no)+"&$top=50&$inlinecount=allpages";   
        ...
        {            
            ...
            var disclaimer="";var LUD="";var RKEY="";
            for (var i = 0; i < data.results.length; i++) 
            {	
                LUD=data.results[0].LUD;
                RKEY=data.results[0].RKEY;
                ...
                jQuery('#discl_info').html('"Disclaimer :'+disclaimer+'-:'+lastUpdate_date+'"');
